...
func (service *BotService) GetUserCurrentActiveChannel(db *sql.DB, app *config.App, bot *tb.Bot, m *tb.Message, userID int) *models.Channel {
	...
	if err := db.QueryRow("SELECT ch.uniqueID,ch.id,ch.channelID,ch.channelName,ch.manualChannelName,ch.channelURL,ch.channelType,us.id,us.userID,cc.companyID,com.companyName,IFNULL(cse.joinVerify,0) as joinVerify,IFNULL(cse.newMessageVerify,0) as newMessageVerify,IFNULL(cse.replyVerify,0) as replyVerify,IFNULL(cse.directVerify,0) as directVerify from `channels` as ch inner join `users_current_active_channel` as uc on ch.id=uc.channelID and uc.status='ACTIVE' inner join `users` as us on uc.userID=us.id and us.userID=? and us.`status`='ACTIVE' inner join companies_channels as cc on cc.channelID=ch.id inner join companies as com on com.id=cc.companyID left join channels_settings as cse on cse.channelID=ch.id", userID).Scan(&channelModel.UniqueID, &channelModel.ID, &channelModel.ChannelID, &channelModel.ChannelName, &channelModel.ManualChannelName, &channelModel.ChannelURL, &channelModel.ChannelType, &userModel.ID, &userModel.UserID, &companyModel.ID,&companyModel.CompanyName, &joinVerify, &newMessageVerify, &replyVerify, &directVerify); err != nil {
		log.Println(err)
		return channelModel
	}
	if joinVerify == 0 {
