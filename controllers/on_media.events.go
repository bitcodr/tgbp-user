...
func onMediaEvents(app *config.App, bot *tb.Bot) {
	...
	if lastState.State == config.LangConfig.GetString("STATE.NEW_MESSAGE_TO_GROUP") ||
		lastState.State == config.LangConfig.GetString("STATE.REPLY_TO_MESSAGE") ||
		lastState.State == config.LangConfig.GetString("STATE.REPLY_BY_DM") ||
		lastState.State == config.LangConfig.GetString("STATE.ANSWER_TO_DM") {
